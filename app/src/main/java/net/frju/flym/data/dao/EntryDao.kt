...
abstract class EntryDao {
    ...
    @Query("UPDATE entries SET favorite = 1 WHERE id IN (:ids)")
    abstract fun markAsFavorite(ids: List<String>)

    @Query("UPDATE entries SET favorite = 0 WHERE id IS :id")
    abstract fun markAsNotFavorite(id: String)

    @Query("UPDATE entries SET favorite = 0 WHERE id IN (:ids)")
    abstract fun markAsNotFavorite(ids: List<String>)
